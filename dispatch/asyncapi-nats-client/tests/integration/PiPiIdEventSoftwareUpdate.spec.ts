...
describe('pi/{pi_id}/event/software_update can talk to itself', () => {
  ...
  it('can send message', async () => {
    ...
    var receivedMsg: Client.SoftwareUpdateRequest | undefined = undefined;
    var receivedPiId: Number | undefined = undefined
    var replyMessage: TestClient.SoftwareUpdateReply = TestClient.SoftwareUpdateReply.unmarshal({
      "status": "Started",
      "detail": "string",
      "exit_code": 0
    });
    var receiveMessage: Client.SoftwareUpdateRequest = Client.SoftwareUpdateRequest.unmarshal({
      "display_name": "string",
      "version": "string",
    ...
    const replySubscription = await testClient.replyToPiPiIdEventSoftwareUpdate((err, msg, pi_id) => {
    ...
    var reply = await client.requestPiPiIdEventSoftwareUpdate(receiveMessage, PiIdToSend);
